...
<html lang="en">
    ...
    <body>
        ...
        <h3>Film berdasarkan kategori</h3>
        <?php include("basisdata.php"); 
            $jenis = $_GET['jenis'];
                $query = "SELECT * FROM dvd WHERE jenis='$jenis'";
            $hasil_query = mysqli_query($conn, $query);
        ?>
        <table class="table">
            <?php while ($baris = mysqli_fetch_array($hasil_query)) { ?>
                <tr>
                <td><img src="img/<?= $baris['nama_gmb']; ?>" height="50px" alt=""></td>
                <td><a href="detail.php?id=<?= $baris['id_film']; ?>"><?= $baris['judul'] ?></a></td>
                </tr>
            <?php } ?>
        </table>
